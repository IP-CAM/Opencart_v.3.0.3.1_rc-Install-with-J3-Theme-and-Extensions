...
class ControllerFeedAdvancedSitemap extends Controller {
      ...
      if ($items) {
        while ($items > 0) {
          ...
          if (!empty($this->request->get['lang'])) {
            $output .= '    <loc>' .$this->url->link('feed/advanced_sitemap/'.$type, 'lang='.$this->request->get['lang'].($page ? '&page='.$page++ : ''), $this->isSSL).'</loc>';
          } else {
            $output .= '    <loc>' .$this->url->link('feed/advanced_sitemap/'.$type, ($page ? 'page='.$page++ : ''), $this->isSSL).'</loc>';
          }
  ...
  public function journal() {
    ...
    foreach ($items as $item) {
      ...
      if ($type == 'category') {
        ...
        $output .= '<loc>' . $this->url->link('product/category', 'path='. ($path ? $path . '_' : '') . $item['category_id']) . '</loc>';
      } else if ($type == 'manufacturer') {
        $output .= '<loc>' . $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $item['manufacturer_id']) . '</loc>';
      } else if ($type == 'information') {
        $output .= '<loc>' . $this->url->link('information/information', 'information_id=' . $item['information_id']) . '</loc>';
      } else {
        $output .= '<loc>' . $this->url->link('product/'.$type, $type.'_id=' . $item[$type.'_id']) . '</loc>';
      }
      ...
      if ($additional_img && $type == 'product' && !empty($item['additional_images']) && $img_loc = $this->model_tool_image->resize($item['image'], $img_width, $img_height)) {
        foreach ($item['additional_images'] as $image) {
          if ($img_loc = $this->model_tool_image->resize($image, $img_width, $img_height)) {
            $output .= '<image:image>';
            $output .= '<image:loc>' . htmlspecialchars($img_loc) . '</image:loc>';
            $output .= '<image:caption>' . htmlspecialchars($item['name']) . '</image:caption>';
            $output .= '<image:title>' . htmlspecialchars($item['name']) . '</image:title>';
  ...
  private function generate_journal($data, $config) {
    ...
    $output .= '<url>';
    $output .= '<loc>' . $this->url->link('journal2/blog', '') . '</loc>';
